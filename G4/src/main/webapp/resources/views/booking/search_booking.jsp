			...
			<div class="row-right col-sm-10" style="background-color: white">
				<div class="container1">
					<div style="padding-bottom: 50px; margin-top: 50px;">
						<div style="float: left;">
							<h1>Employee Office List</h1>
						</div>
						<div class="" style="float: right; padding-right: 20px;">
							<form class="form-inline" action="${pageContext.request.contextPath}/SearchBooking?index=1" method="get">
								<!-- icon -->
								...
								<input class="form-control mr-sm" type="search"
									placeholder="Search" aria-label="Search" name="search" value="${textSearch}">
								<!-- label filter by -->
								<div class="form-control mr-sm"
								...
								<!-- select -->
								<div>
									<select class="form-control mr-sm" name="filterSearch"
										id="listE"
										style="border: 1px solid #e8e8e8; border-radius: 5px;">
										<option value="all" ${filterSearch== 'all'?"selected":""}>-All-</option>
										<option value="name" ${filterSearch== 'name'?"selected":""}>Name</option>
										<option value="trip" ${filterSearch== 'trip'?"selected":""}>Trip</option>
									</select>
								</div>
								<!-- btn search -->
								<button class="btn"
				...
				<div class="container2 ">
					<div class="a" style="background-color: #F8F9FA; font-size: 20px;">
						<div>
							<p style="margin-left: 15px; padding-top: 10px;">List Booking</p>
						</div>

						<hr>
					</div>
					<form action="${pageContext.request.contextPath}/SearchBooking?index=1" method="get">
						...
						<div class="table1"
							...
							<p class="col-12" style="font-style: italic;font-weight: bold">${error}
							<table class="table">
								<thead>
									<tr>
										<th>Id</th>
								...
								<tbody>
									<c:forEach items="${bookingView}" var="x">

										<tr>
											<td>${x.officeld }</td>
											<td>${x.officeName}</td>
											<td>${x.officeName}</td>
											<td><a href="/G4/ViewBooking?officeld=${x.officeld}"><i
						...
						<ul class="pagination">
							<c:forEach begin="1" end="${endP}" var="p">
								<li class="page-item"><a class="page-link"
									href="/G4/SearchBooking?index=${p}&search=${textSearch }&filterSearch=${filterSearch}">${p}</a></li>
							</c:forEach>
						</ul>
					</nav>
				</div>
