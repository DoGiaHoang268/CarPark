			...
			<div class="row">
				...
				<div class="row-right col-sm-10" style="background-color: white">
						...
						<div style="padding-bottom: 50px; margin-top: 50px;">
							...
							<div class="" style="float: right; padding-right: 20px;">
								<form class="form-inline"
									action="${pageContext.request.contextPath}/SearchCarController?sindex=1">
									<!-- icon -->
									...
									<div>
										<select class="form-control mr-sm" name="select" id="select"
											...
											<option value="1" ${select== 1?"selected":""}>License
												Plate</option>
											<option value="2" ${select== 2?"selected":""}>Car
												type</option>
											<option value="3" ${select== 3?"selected":""}>Car
												color</option>
											<option value="4" ${select== 4?"selected":""}>Company</option>
											<option value="5" ${select== 5?"selected":""}>Parking
												lot</option>
										</select>
									</div>
									<!-- btn search -->
					...
					<div class="container2 ">
						...
						<form
							action="${pageContext.request.contextPath}/ListCarController">
									...
									<tbody>
										<c:forEach items="${listC}" var="x">
											<tr>
												...
												<td>${x.carColor}</td>
												<td>${x.company}</td>
												<td><a href="">Bai so ${x.parkingLot}</a></td>
												<td><a
													href="${pageContext.request.contextPath}/EditCarController?licenseplate=${x.licenseplate}"><i
														class="bi bi-pencil-square"></i> Edit</a>&emsp; <a
													href="${pageContext.request.contextPath}/DeleteCarController?licenseplate=${x.licenseplate}"
							...
							<ul class="pagination">
								<c:forEach begin="1" end="${endP}" var="p">
									<li class="page-item"><a class="page-link"
										href="${pageContext.request.contextPath}/SearchCarController?index=${p}&search=${save}">${p}</a></li>
								</c:forEach>
							</ul>
						</nav>
					</div>
