							...
							<form class="form-inline" action="SearchParking">
								...
								<div>
									<select class="form-control mr-sm" name="filterSearch" id="listE"
										...
										<option value="parkName" ${filterSearch== 'parkName'?"selected":""}>Park Name</option>
										<option value="parkPlace" ${filterSearch== 'parkPlace'?"selected":""}>Park Place</option>
										<option value="parkArea" ${filterSearch== 'parkArea'?"selected":""}>Park Area</option>
										<option value="parkPrice" ${filterSearch== 'parkPrice'?"selected":""}>Park Price</option>
										<option value="parkStatus" ${filterSearch== 'parkStatus'?"selected":""}>Park Status</option>
									</select>
								</div>
								<!-- btn search -->
								<button class="btn"
								...
								<tbody>
									<c:forEach items="${parking}" var="x">
										<tr>
											...
											<td>${x.price}</td>
											<td>${x.status}</td>
											<td><a href="/G4/EditParking?parkId=${x.parkId}"><i
													class="bi bi-pencil-square"></i>Edit</a>&emsp; <a href=""><i
						...
						<ul class="pagination">
							<c:forEach begin="1" end="${endP}" var="p">
								<li class="page-item"><a class="page-link"
									href="/G4/SearchParking?index=${p}&search=${textSearch }&filterSearch=${filterSearch}">${p}</a></li>
							</c:forEach>
						</ul>
					</nav>
				</div>
