			...
			<div class="row">
				...
				<div class="row-right col-sm-10" style="background-color: white">
								...
								<form class="form-inline"
									...
									<div>
										<select class="form-control mr-sm" name="select" id="select"
											...
											<option value="1" ${select== 1?"selected":""}>Name</option>
											<option value="2" ${select== 2?"selected":""}>Date
												of birth</option>
											<option value="3" ${select== 3?"selected":""}>Address</option>
											<option value="4" ${select== 4?"selected":""}>Phone</option>
											<option value="5" ${select== 5?"selected":""}>Department</option>
										</select>
									</div>
									<!-- btn search -->
									<button class="btn"
									...
									<tbody>
										<c:forEach items="${listEm}" var="x">
											<tr>
												...
												<td>${x.department}</td>
												<td><a
													href="${pageContext.request.contextPath}/editEmployee?eId=${x.employeeId}"><i
														class="bi bi-pencil-square"></i> Edit</a>&emsp;&emsp; <a
					...
					<div class="navigation">
						<nav>
							<ul class="pagination">
								<c:forEach begin="1" end="${endP1}" var="p">
									<li class="page-item"><a class="page-link"
										href="${pageContext.request.contextPath}/searchEmployee?sindex1=${p}&search=${save}">${p}</a></li>
								</c:forEach>
							</ul>
						</nav>
					</div>
